...
namespace _05.Rubik_s_Matrix
{
    class Program
    {
        static void Main()
        {
            ...
            var Print = new int[sizes[0], sizes[1]];
            var number = 1;
            ...
            for (int i = 0; i < matrix.GetLength(0); i++)
            {
                for (int j = 0; j < matrix.GetLength(1); j++)
                {
                    if (matrix[i, j] != Print[i, j])
                    {
                        EqualizeValue(matrix, Print, ref i, ref j);
                    }
                    else
                    {
        ...
        private static void EqualizeValue(int[,] matrix, int[,] Print, ref int X, ref int Y)
        {
            var swap = 0;
            for (int i = 0; i < matrix.GetLength(0); i++)
            {
                for (int j = 0; j < matrix.GetLength(1); j++)
                {
                    if (matrix[i, j] == Print[X, Y])
                    {
                        Console.WriteLine($"Swap ({X}, {Y}) with ({i}, {j})");
                        swap = matrix[X, Y];
